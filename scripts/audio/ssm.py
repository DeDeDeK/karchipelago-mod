...
class Channel:
    ...
    def pack(self):
        return (
            struct.pack(">HHIII", self.loop, self.fmt, self.sa, self.ea, self.ca)
    ...
    def unpack(cls, b, o):
        ...
        gain, ips, yn1, yn2, lps, lyn1, lyn2, pad = struct.unpack_from(
            ">HHhhHhhH", b, o + 48
        )
        return cls(
            loop=loop,
...
class SSM:
    ...
    def load(cls, path):
        ...
        for _ in range(count):
            ...
            chans = [
                Channel.unpack(b, o + ENTRY_FIXED + i * CHANNEL_SIZE)
                for i in range(nch)
            ]
        ...
        if o - 0x10 != table_size:
            raise ValueError(
                f"{path}: entry table is {o - 0x10:#x}, header says {table_size:#x}"
            )
        return cls(base, sounds, b[data_off : data_off + data_size])

    def pack(self):
        table = b"".join(
            struct.pack(">2I", len(s.channels), s.sample_rate)
            + b"".join(c.pack() for c in s.channels)
            for s in self.sounds
        )
        pad = -(0x10 + len(table)) % DATA_ALIGN
        data = self.data + b"\0" * (-len(self.data) % DATA_ALIGN)
        return (
            struct.pack(">4I", len(table), len(data), len(self.sounds), self.base_index)
            + table
            + b"\0" * pad
            + data
        )
