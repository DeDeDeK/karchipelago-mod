...
TOBJ_COORD = {0: 'UV', 1: 'REFLECTION', 2: 'HILIGHT', 3: 'SHADOW', 4: 'TOON', 5: 'GRADATION'}
# Bits 16-19: colormap blend mode.
TOBJ_COLORMAP = {1: 'CM_ALPHA_MASK', 2: 'CM_RGB_MASK', 3: 'CM_BLEND',
                 4: 'CM_MODULATE', 5: 'CM_REPLACE', 6: 'CM_PASS',
                 7: 'CM_ADD', 8: 'CM_SUB'}
# Bits 20-23: alphamap blend mode.
TOBJ_ALPHAMAP = {1: 'AM_ALPHA_MASK', 2: 'AM_BLEND', 3: 'AM_MODULATE',
                 4: 'AM_REPLACE', 5: 'AM_PASS', 6: 'AM_ADD', 7: 'AM_SUB'}
...
GX_COMPARE      = {0: 'NEVER', 1: 'LESS', 2: 'EQUAL', 3: 'LEQUAL',
...
FOG_TYPES = {
    0x00: 'NONE',
    0x02: 'PERSP_LINEAR', 0x04: 'PERSP_EXP',   0x05: 'PERSP_EXP2',
    0x06: 'PERSP_REVEXP', 0x07: 'PERSP_REVEXP2',
    0x0A: 'ORTHO_LINEAR', 0x0C: 'ORTHO_EXP',   0x0D: 'ORTHO_EXP2',
    0x0E: 'ORTHO_REVEXP', 0x0F: 'ORTHO_REVEXP2',
}
...
GX_TEX_FILTER = {0: 'NEAR', 1: 'LINEAR', 2: 'NEAR_MIP_NEAR', 3: 'LIN_MIP_NEAR',
                 4: 'NEAR_MIP_LIN', 5: 'LIN_MIP_LIN'}
...
TREE_FIELDS = {
    'JOBJDesc':     [(0x08, 'JOBJDesc'),    # child
                     (0x10, 'DObjDesc'),    # dobj (may be Spline/ParticleJoint)
                     (0x3C, 'RObjDesc'),    # robj
                     (0x0C, 'JOBJDesc')],   # next  (printed last)
    'DObjDesc':     [(0x08, 'MObjDesc'),
                     (0x0C, 'POBJDesc'),
                     (0x04, 'DObjDesc')],
    'MObjDesc':     [(0x08, 'TObjDesc'),
                     (0x0C, 'MaterialDesc'),
                     (0x14, 'PEDesc')],
    'TObjDesc':     [(0x4C, 'ImageDesc'),
                     (0x50, 'TlutDesc'),
                     (0x54, 'TexLODDesc'),
                     (0x58, 'TObjTev'),
                     (0x04, 'TObjDesc')],
    'POBJDesc':     [(0x04, 'POBJDesc')],  # +0x14 routed dynamically
    'RObjDesc':     [(0x08, 'JOBJDesc'),    # only when REFTYPE == JOBJ
                     (0x00, 'RObjDesc')],   # next
    'LObjDesc':     [(0x10, 'WObjDesc'),
                     (0x14, 'WObjDesc'),
                     (0x04, 'LObjDesc')],
    'WObjDesc':     [(0x10, 'RObjDesc')],
    'FogDesc':      [(0x04, 'FogAdjDesc')],
    'FogAnim':      [(0x00, 'FogDesc'),
                     (0x04, 'AOBJ')],
    'AOBJ':         [(0x08, 'FOBJDesc'),
                     (0x0C, 'JOBJDesc')],   # ObjectReference (often NULL)
    'FOBJDesc':     [(0x00, 'FOBJDesc')],   # linked list; buffer at +0x10 is leaf
    'FOBJ':         [],
    'LightGroup':   [(0x00, 'LightNode'),
                     (0x04, 'LightNode'),
                     (0x08, 'LightNode')],
    'LightNode':    [(0x00, 'Light'),
                     (0x04, 'Light'),
                     (0x08, 'Light'),
                     (0x0C, 'Light')],
    'Light':        [(0x00, 'LObjDesc')],
    # SOBJ: three NULL-terminated pointer arrays + an inline FogAnim.
    'SOBJ':         [(0x00, 'ModelGroup', 'array'),
                     (0x04, 'Camera', 'array'),
                     (0x08, 'Light', 'array'),
                     (0x0C, 'FogAnim')],
    'ModelGroup':   [(0x00, 'JOBJDesc')],
    'Camera':       [(0x18, 'WObjDesc'),    # eye
                     (0x1C, 'WObjDesc')],   # target
...
def _detail(arc: Archive, typ: str, off: int) -> str:
    ...
    if typ == 'MaterialDesc':
        ...
        def rgba(t): return f'#{t[0]:02X}{t[1]:02X}{t[2]:02X}{t[3]:02X}'
    ...
    if typ == 'PEDesc':
        ...
        return (f' [{_decode_pe_flags(peflags)}]'
    ...
    if typ == 'LObjDesc':
        ...
        return (f' color=#{r:02X}{g:02X}{b:02X}{a:02X}'
                f' [{_decode_lobj_flags(flags)}{attn_tag}]')
    if typ == 'WObjDesc':
        return (f' pos=({f32(arc.data, off+0x04):.2f},'
    ...
    if typ == 'TexLODDesc':
        ...
        return (f' min={GX_TEX_FILTER.get(minf, minf)}'
                f' bias={bias:+.2f}')
    if typ == 'TObjTev':
    ...
    if typ == 'SOBJ':
        ...
        def _arr_len(slot):
            ...
            while True:
                ...
                i += 1
            return i
        return (f' models={_arr_len(0x00)}'
                f' cameras={_arr_len(0x04)}'
                f' lights={_arr_len(0x08)}')
    if typ == 'Camera':
        nm = _name_at(arc, off, 0x00)
    ...
    if typ == 'AOBJ':
        ...
        return (f' flags={flags:#010x} [{_decode_aobj_flags(flags)}]'
    ...
    if typ == 'FOBJDesc':
        ...
        return (f' track={track} start={start:.1f} len={data_len}B'
...
def _walk_print(arc: Archive, root: int, root_type: str, max_depth):
    ...
    sizes_known = {
        ...
        'POBJDesc': 0x18, 'RObjDesc': 0xC,  'ImageDesc': 0x18, 'TlutDesc': 0x10,
        'MaterialDesc': 0x14, 'PEDesc': 0xC, 'TexLODDesc': 0x10, 'TObjTev': 0x20,
        'LObjDesc': 0x1C, 'WObjDesc': 0x14, 'FogDesc': 0x18, 'FogAdjDesc': 0x44,
        'LightGroup': 0xC, 'LightNode': 0x10, 'Light': 0x08,
        'Spline': 0x18, 'ParticleJoint': 0x08,
        'SOBJ': 0x10, 'Camera': 0x40, 'ModelGroup': 0x10, 'FogAnim': 0x08,
        'ShapeSet': 0x1C, 'IOBJDesc': 0x0C,
        'AOBJ': 0x10, 'FOBJDesc': 0x14, 'FOBJ': 0x08,
    }
    ...
    def go(depth, label, typ, off):
        ...
        if off in visited:
            line(depth, label, typ, off, f' (cycle, first seen at depth {visited[off]})')
            return
        visited[off] = depth
...
def _print_stage_node(arc: Archive, sn: int):
    ...
    print(f"    Gravity      = ({f32(arc.data, sn+0x10):.4f}, "
    ...
    print(f"    OoB min      = ({f32(arc.data, sn+0xCC):.1f}, "
          f"{f32(arc.data, sn+0xD0):.1f}, {f32(arc.data, sn+0xD4):.1f})")
    print(f"    OoB max      = ({f32(arc.data, sn+0xD8):.1f}, "
...
def _print_light_group(arc: Archive, lg: int):
    ...
    for i, lname in enumerate(['Global', 'Group1', 'Group2']):
        ...
        for j in range(4):
            ...
            print(f"        Light[{j}] @ {lp:#x} -> LObjDesc @ {lobj:#x}"
                  if lobj else f"        Light[{j}] @ {lp:#x} (no LObj)")
            if lobj:
...
def _print_fog_node(arc: Archive, fn: int):
    ...
    print(f"    +00  FogData  -> {_fmt_ptr(fog_data_p, fog_data_slot in arc.reloc_set)}")
    if fog_data_p:
...
def cmd_grdata(args):
    ...
    if sym is None:
        ...
        print(f"public symbol {sym!r} not found. "
              f"Available: {list(arc.publics)}", file=sys.stderr)
        return 1
