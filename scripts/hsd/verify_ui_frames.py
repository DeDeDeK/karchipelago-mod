...
"""Check that splicing CmUiFrames.dat in reproduces a rewritten donor exactly.

custom_machines patches the appended UI frames into each vanilla menu archive as
...
actually read out of both: every bank's image count, TLUT count, appended images
(dimensions, format and texels) and decoded image-index and TLUT ramps.

Only the placeholder is compared. A registered machine's own art replaces it in
one slot at runtime, out of its own .art side-car, which neither path here models.
...
from hsd.make_ui_frames import BANK_SIZE, FILE_SIZE, PUBLIC, RAMP_SIZE
...
def sidecar_appended(file):
    """Frames the side-car's ramps were authored to add. Uniform across banks."""
    return file["banks"][0]["n_appended"]


def read_sidecar(path):
    """{donor name: {"banks": [...], "ramps": [...]}} out of the side-car archive."""
    arc = Archive(path)
    d = arc.data
    ...
    while True:
        ...
        for b in range(n_banks):
            ...
            rows.append({
                ...
                "n_appended": u16(d, o + 0x0A), "image": u32(d, o + 0x0C),
                "timg": u32(d, o + 0x10), "timg_buf": u32(d, o + 0x14),
                "timg_len": u32(d, o + 0x18),
                "tclt": u32(d, o + 0x1C), "tclt_buf": u32(d, o + 0x20),
                "tclt_len": u32(d, o + 0x24),
                "timg_flag": d[o + 0x30], "tclt_flag": d[o + 0x31],
            })
        n_ramps, ramps = u32(d, at + 0x0C), u32(d, at + 0x10)
        joints = []
        for b in range(n_ramps):
            o = ramps + b * RAMP_SIZE
            joints.append({
                "fobj": u32(d, o), "buf": u32(d, o + 0x04),
                "len": u32(d, o + 0x08), "flag": d[o + 0x0C],
            })
        files[name] = {"banks": rows, "ramps": joints}
        i += 1
    return arc, files


def splice(donor, side, rows, joints):
    ...
    for r in rows:
        ...
        tbl = len(data)
        data.extend(struct.pack(f">{n + a}I", *imgs, *([base + r["image"]] * a)))
        struct.pack_into(">I", data, tex + auf.TEXANIM_IMAGES, tbl)
        struct.pack_into(">H", data, tex + auf.TEXANIM_N_IMAGES, n + a)

        if r["tclt"]:
            nt = u16(data, tex + auf.TEXANIM_N_TLUTS)
            tluts = [u32(data, u32(data, tex + auf.TEXANIM_TLUTS) + i * 4) for i in range(nt)]
            tbl = len(data)
            data.extend(struct.pack(f">{nt + a}I", *tluts, *([tluts[r["tlut_src"]]] * a)))
            struct.pack_into(">I", data, tex + auf.TEXANIM_TLUTS, tbl)
            struct.pack_into(">H", data, tex + auf.TEXANIM_N_TLUTS, nt + a)

        for off, buf, ln, flag in ((r["timg"], r["timg_buf"], r["timg_len"], r["timg_flag"]),
                                   (r["tclt"], r["tclt_buf"], r["tclt_len"], r["tclt_flag"])):
            if off:
                struct.pack_into(">I", data, off + auf.FOBJ_LENGTH, ln)
                struct.pack_into(">I", data, off + auf.FOBJ_BUFFER, base + buf)
                data[off + auf.FOBJ_VALUE_FLAG] = flag

    for j in joints:
        struct.pack_into(">I", data, j["fobj"] + auf.FOBJ_LENGTH, j["len"])
        struct.pack_into(">I", data, j["fobj"] + auf.FOBJ_BUFFER, base + j["buf"])
        data[j["fobj"] + auf.FOBJ_VALUE_FLAG] = j["flag"]
...
def main(argv):
    ...
    p.add_argument("--sidecar", default="mods/custom_machines/assets/CmUiFrames.dat")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for name, file in files.items():
            ...
            for j in file["ramps"]:
                want = [(k.frame, k.value, k.op) for k in auf.ramp_keys(grown.data, j["fobj"])]
                got = [(k.frame, k.value, k.op) for k in auf.ramp_keys(spliced, j["fobj"])]
                if want != got:
                    bad += 1
                    print(f"  MISMATCH {name} joint ramp {j['fobj']:#x}:")
                    print(f"    rewrite {want}")
                    print(f"    splice  {got}")
    print(f"\n{'FAIL' if bad else 'OK'}: {sum(len(f['banks']) for f in files.values())} "
          f"bank(s) and {sum(len(f['ramps']) for f in files.values())} joint ramp(s) "
          f"across {len(files)} archive(s), {bad} mismatch(es)")
