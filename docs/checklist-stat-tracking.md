...
condition is met - is the clear-checker's own concern and is not covered here.
...
3. **Clear-bit storage / detection** - `ClearChecker_SetNewUnlock` flips the
   `GameClearData.clear[clear_kind]` bit.
...
| `0x8022f920` | `Ply_GetItemCollectTotal(player)` | `sum collect[3..0x43]` - items picked up this game, **boxes excluded**. |
| `0x8022f9bc` | `Ply_GetBoxCollectTotal(player)` | `sum collect[0..2]` - boxes collected/broken this game. |
...
|----|-----------|------|-----------|
| 0x00 | race over 60 miles | FNU | `records+0x14` (sum dist) -> miles `= raw*11.4286/160934 >= 60.0` |
| 0x01 | race over 200 miles | FNU | same `>= 200.0` |
| 0x02 | pick up >100 items | FNU | `records+0xe` (sum `Ply_GetItemCollectTotal`) `>= 100` |
| 0x03 | >500 items | FNU | `records+0xe >= 500` |
| 0x04 | >1000 items | FNU | `records+0xe >= 1000` |
| 0x05 | >3000 items | FNU | `records+0xe >= 3000` |
| 0x06 | Free Run: change machines 10x | FR | `sum +0x37c[26] >= 10` (per human) |
| 0x07 | break >500 boxes | FNU | `records+0x10` (sum `Ply_GetBoxCollectTotal`) `>= 500` |
| 0x08 | >1000 boxes | FNU | `records+0x10 >= 1000` |
| 0x09 | Free Run: drive 10 min | FR | `(sum_human (+0x5e4 + +0x5e8)) + records+0x18 >= 36000` |
| 0x0A | drive 30 min | FR | `>= 108000` |
| 0x0B | drive 2 hours | FR | `>= 432000` |
| 0x0C | play >10 stadium modes | SP, ST | `sum_{0..23} Gm_StadiumIsAvailable(i) >= 10` |
...
| 0x1E | TF total > 1500 pts | SS | `records+0x8` (sum TF pts, u16) `>= 1500` |
...
| 0x32 | break 5+ huge pillars | FNU | `records-0x3` (sum_human byte[0x28]) `>= 5` |
| 0x33 | break pillar within 40 s | FNU | `+0x830` (valid `+0x653`) `>= 0 && <= 2400` |
| 0x34 | steal >8 items from Tac | FNU | `+0x808 >= 8` |
| 0x35 | meteor attacks 3x | FNU | `records-0x2` (sum global event-count `CityEvent_GetOccurrenceCount(2)`) `>= 3` |
...
| 0x3b | bust star pole 10x | FNU | `records-0x1` (sum_human byte[0x1d]) `>= 10` |
| 0x3c | open all volcano-base holes | FNU | `sum_present byte[0x25] >= 3` |
| 0x3d | waterwheel carry 10x | FNU | `records+0x6` (sum_human `+0x820`) `>= 10` |
| 0x3e | reach the sky garden | FNU | `+0x84d bit1` |
| 0x3f | open forest pitfall | FNU | `byte[0x20] != 0` |
| 0x40 | high-plains hole 3x | FNU | `sum_human +0x834 >= 3` |
| 0x41 | break all volcano+highplains rocks | FNU | `sum_present byte[0x23] >= 41` |
| 0x42 | grind-rail into crater | FNU | `+0x850 != 0` |
| 0x43 | super-jump onto building 10x | FNU | `records+0x4` (sum_human `+0x838`) `>= 10` |
| 0x44 | destroy all dilapidated houses | FNU | `sum_present byte[0x26] >= 30` |
| 0x45 | knock down all forest trees | FNU | `sum_present byte[0x22] >= 53` |
...
| 0x4b | timeout while all off machines | FNU | present-count `== sum +0x854 bit3` |
| 0x4c | timeout while all on rails | FNU | present-count `== sum +0x854 bit4` |
| 0x4d | break CPU machine 5x | FNU | `records+0x5` (sum_human `+0x4b4`) `>= 5` |
...
| 0x5f | Gold Spike KO 3x | FNU | `records+0x1` (sum_human `+0x4bc`) `>= 3` |
| 0x60 | Firework KO 10x | FNU | `records+0x2` (sum_human `+0x4b8`) `>= 10` |
...
| `-0x3` | `+0xF5` | u8 | huge pillars broken (sum byte[0x28]) | 255 | 0x32 |
| `-0x2` | `+0xF6` | u8 | meteor strikes (sum event-count[2]) | 255 | 0x35 |
| `-0x1` | `+0xF7` | u8 | star-pole busts (sum byte[0x1d]) | 255 | 0x3b |
...
ST's jump table is at `0x80497738` (12 x u32, indexed `(city_kind - 7)`).
...
| `0x8022f19c` | `Ply_GetMachineChangeCount(player)` | `sum +0x37c[26]` |
...
`sum_human` value into the running total with plain `int` adds - **no saturating
caps**, unlike CT's u8/u16 records.

| records | abs | role (per-game source summed) | writer | drives |
|---|---|---|---|---|
| `-0x4` | `0x80536834` | total laps (`sum GameData+0x868[p]`) | `0x8004ab90` | 0x00 / 0x01 |
| `+0x0` | `0x80536838` | total glide time, frames (`sum +0x5e8`) | `0x8004acf4` | 0x02 / 0x03 |
| `+0x4` | `0x8053683C` | total enemies defeated (`sum (+0xe0)+(+0x6a0)`) | `0x8004ae50` | 0x04 / 0x05 |
| `+0x8` | `0x80536840` | goals reached (`# humans finished`) | `0x8004b1d0` | 0x0a |
| `+0xc` | `0x80536844` | enemies swallowed (`sum +0x6a0`) | `0x8004b2b8` | 0x0e |
| `+0x10` | `0x80536848` | exhaled-star defeats (`sum defeat[0xf]+defeat[0x15]`) | `0x8004b404` | 0x0f |
| `+0x14` | `0x8053684C` | Quick-Spin defeats (`sum defeat[0x10]`) | `0x8004b814` | 0x1f |
| `+0x18` | `0x80536850` | Quick-Spin rival hits (`sum hit[0x10]`) | `0x8004b968` | 0x20 |
...
| ck | Objective | Eval | Condition |
|----|-----------|------|-----------|
| 0x00 | race over 100 laps | A-finish | `records-0x4 (sum laps) >= 100` |
| 0x01 | race over 300 laps | A-finish | `records-0x4 >= 300` |
| 0x02 | glide > 30 min | B-frame | `sum_human +0x5e8 + records+0x0 >= 108000` (30 min) |
...
| 0x0a | reach goal 3x (non-Free Run) | A-finish | `records+0x8 (goals) >= 3` |
| 0x0e | swallow 200+ enemies | B-frame | `sum +0x6a0 + records+0xc >= 200` |
| 0x0f | defeat 100+ with exhaled stars | B-frame | `sum (+0xe4[0xf]+[0x15]) + records+0x10 >= 100` |
| 0x10 | swallow 5 consecutive garbage enemies & 1st | A-finish | `+0x7c8 >= 5` & 1st |
| 0x1c | sword challenge: swing exactly 10x & 1st | A-finish | `+0x7d4 == 10` & 1st |
| 0x1e | tornado challenge: 15+ KOs & 1st | A-finish | `+0x7d8 >= 15` & 1st |
| 0x1f | 10+ Quick Spin defeats | B-frame | `sum +0xe4[0x10] + records+0x14 >= 10` |
| 0x20 | hit 20+ rivals with Quick Spin | B-frame | `sum +0x74[0x10] + records+0x18 >= 20` |
...
| 0x5e | drop from cliffs 3x in one game | B-frame | any human `+0x810 >= 3` |
...
| 0x63 | CHECKER KNIGHTS: spin panels 7x & 1st | A-finish | gr 3, `+0x7f4 >= 7` & 1st |
...
| 0x68 | SKY SANDS: enter quicksand 3x & 1st | A-finish | gr 2, `+0x814 == 3` & 1st |
| 0x6a | SKY SANDS: open trapdoor exactly 3x & 1st | A-finish | gr 2, `+0x7fc == 3` & 1st |
...
**Distance - "race over N feet in 2 minutes"** (`AirRide_CheckRaceDistanceObjectives`,
`0x8004d454`). "In 2 minutes" is the **configured race time limit**, not a mid-race
sample: the gate is `Gm_GetRaceTimeLimitSeconds()` (`0x800091e4`, `GameData+0xa9c`)
`* 60 == 7200`, so the player has to run a 2:00.00 timed race. Per human slot it
reads `Gm_GetPlayerRaceDistance(p)` (`0x80009568`, metres) live out of `GameData`
rather than the latched results block, divides by `0.3047879934`
(`FLOAT_805de928`) and awards on `>=` the threshold, with
`ClearChecker_GetKindClear & 5` suppressing a repeat.

| ck | stage | gr (GroundKind) | feet |
|----|-------|-----------------|------|
| 0x14 | Fantasy Meadows | 0 `GR_PLANTS1` | 4500 |
| 0x0c | Magma Flows | 1 `GR_HEAT2` | 4800 |
| 0x16 | Sky Sands | 2 `GR_DESERT1` | 4000 |
| 0x0b | Checker Knights | 3 `GR_CHECK2` | 5500 |
| 0x15 | Celestial Valley | 4 `GR_VALLEY2` | 6000 |
| 0x21 | Machine Passage | 5 `GR_MACHINE2` | 4500 |
| 0x22 | Beanstalk Park | 7 `GR_SKY2` | 5500 |
| 0x17 | Frozen Hillside | 8 `GR_ICE1` | 5300 |

Nebula Belt (gr 6 `GR_SPACE2`) has no cell of this kind: its jump-table slot points
straight at the loop, leaving the required-frames local at 0 so the objective can
never fire.
...
| 0x0e | get the same item 3x in one race | per-course | item-repeat flag set |
...
| 0x18 | hit enemies 3x with Bomb items | per-course-B | Bomb-mgr+0x1c `>= 3` |
...
take 1st 10x, **+4** finish N laps under T, **+5** race 100+ laps, then the
...
+2 a CPU at level value 4 (= "level 5") finished, read out of the per-slot Top Ride
config block (`gmGetTopRideConfigP` -> `0x805366a0` = `GameData+0xcc8`, slots start
at `+0x58` with a 9-byte stride; `+0` = slot active, `+3` = CPU level); +3
`stats+0x9[course] >= 10`;
...
| course | base | no-items / no-Boost / CPU5 / 10x / laps<T / 100laps | gimmick cells | 5 s faster |
|--------|------|----------------------------------------------------|---------------|------------|
| Grass | 0x1a | 0x1a / 0x1b / 0x1c / 0x1d / 0x1e (7 laps 00:43) / 0x1f | 0x20 hit 5 Dash Panels, 0x21 drop 30 tree bombs | 0x22 |
| Sand | 0x23 | 0x23 / 0x24 / 0x25 / 0x26 / 0x27 (7 laps 00:52) / 0x28 | 0x29 catch worm 3x, 0x2a ant-doom 50x, 0x2b ant-doom 20x one game | 0x2c |
| Sky | 0x2d | 0x2d / 0x2e / 0x2f / 0x30 / 0x31 (6 laps 01:02) / 0x32 | 0x33 hit Isle Knob 5x, 0x34 1st without Jump Plate | 0x35 |
| Fire | 0x36 | 0x36 / 0x37 / 0x38 / 0x39 / 0x3a (6 laps 00:53) / 0x3b | 0x3c huge eruption 3x, 0x3d 1st holding Fire item | 0x3e |
| Water | 0x3f | 0x3f / 0x40 / 0x41 / 0x42 / 0x43 (5 laps 01:02) / 0x44 | 0x45 enter falls 5x | 0x46 |
| Light | 0x47 | 0x47 / 0x48 / 0x49 / 0x4a / 0x4b (6 laps 00:43) / 0x4c | 0x4d grind rail 50x, 0x4e grind rail 5x, 0x4f bust 6 columns | 0x50 |
| Metal | 0x51 | 0x51 / 0x52 / 0x53 / 0x54 / 0x55 (5 laps 00:58) / 0x56 | 0x57 1st without breaking gear walls, 0x58 hit switch 10x, 0x59 break 5 gear walls | 0x5a |
