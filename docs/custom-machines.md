...
| `SetStarInitHandler(kind, fn)` / `SetStarThinkHandler(kind, fn)` | claim the engine's own per-kind slots on the star class, for per-machine work a consumer cannot hook itself |
| `GetMachineJoint(md, index)` | the live joint for a depth-first index into the archive's joint tree, the numbering `palette_joint` uses |
| `GetPalette(kind, &count)` | the descriptor's palette, for a consumer that wants the same colors the joint cycle walks |
