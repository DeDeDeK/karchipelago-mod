...
A descriptor whose `version` is not `CUSTOM_MACHINE_DESC_VERSION` is rejected at discovery and reported. `clone_machine.py` always writes the current one, so a mismatch means a stale `.dat` left behind by an older build rather than a descriptor to be read partially.

Registration order is FST scan order, and it decides the appended kinds: the *n*-th machine found takes star slot `19 + n` and `MachineKind` `VCKIND_NUM + n`. `CharacterKind`s come off a separate counter that only advances for a machine that asked for one, so they run `CKIND_NUM` upward in the order of the machines with `wants_character`, not in registry order. `CUSTOM_MACHINE_MAX` caps the registry at 13, which is what the select screens are widened by: their relocated icon lists hold 33 entries, 20 of them the vanilla roster. The same 13 is the width the UI art banks are grown by and the width the per-machine counter arrays are relocated to, so all three move together.

Discovery runs at `OnBoot`, inside the window where hoshi has swapped `HSD_MemAlloc` and `Archive_LoadFile` for versions that allocate out of a persistent bump arena over the HSD heap. So each candidate loads through the ordinary `Archive_LoadFile`, and the mod brackets it with `HSD_ArenaMark` / `HSD_ArenaRelease` to rewind that arena once the descriptor is read. Everything the registry keeps - the strings, the palette colors, the cutscene archive names - is copied out of the descriptor first. A machine archive runs to six figures of bytes and the engine loads its own copy later, by filename, through the widened name table, so holding the boot copy would cost over a megabyte at the registry's cap for a few hundred bytes of descriptor.
...
A character is only appended while the select screens still have an icon for it, so `GetSelectIconMax()` is the real cap on appended characters even though `CUSTOM_MACHINE_MAX` machines can register. A machine past it still loads, drives, spawns in City Trial and can be handed out; it just is not on the grids. Art is not part of that decision: the side-car authors its ramps a fixed `CUSTOM_MACHINE_MAX` frames wide whether or not a machine claims each slot, and an unclaimed slot holds the placeholder, so a machine is never refused a cell for want of a portrait.
...
- The packed icon list in `GameData` - a count at select base `+0x65` and one `CharacterKind` per icon from `+0x66` - is followed immediately by live bytes: Air Ride's row-layout flag at `+0x7a` and its debug-grid flag at `+0x7b`, and City Trial's debug-grid flag at `+0x7a` in its own block. Each moves to the far end of its screen's free span by rewriting the displacement of every `lbz`/`stb` that reaches it - Air Ride's row-split to `+0x87` across nine sites (`0x80020b04`, `0x80020b4c`, `0x80020b98`, `0x800214a4`, `0x80027f60`, `0x800285c8`, `0x80028818`, `0x80028970`, `0x80029c7c`), its debug-grid to `+0x88` across six (`0x80020a88`, `0x8002881c`, `0x8002895c`, `0x80028968`, `0x80029c64`, `0x80029c70`), and City Trial's to `+0x87` across six (`0x8002e444`, `0x80038d00`, `0x8003a150`, `0x8003a15c`, `0x8003ac3c`, `0x8003ac48`). Code that rebuilds the Air Ride list has to write the flag through `SetAirRideRowSplit` rather than at the vanilla offset.
  ...
  The list then runs `base+0x66` through `base+0x86` - 33 entries, 20 of them the vanilla roster. That span is not free: it covers four per-slot 4-byte arrays each screen's think function reads, at City Trial's `base+0x7b`, `+0x7f`, `+0x83` and `+0x87` and Air Ride's `+0x7c`, `+0x80`, `+0x84` and `+0x88`. `CitySelect_Think+0x98` (`0x80037b28`) walks `base[0x83 + ply]`, a per-player save-file index into a `0x110`-stride checklist table, and `base[0x87 + ply]`, a latch whose `-1` means nothing is pending; `CSS_airRide_inputGrabber+0xa10` (`0x80027940`) does the same at `+0x84` and `+0x88`. Packing a list clears all 33 bytes, so those arrays are zeroed on every rebuild of either screen, and the relocated flags land on the first byte of one of them.
...
- **A vanilla legendary has not already run this scene.** The engine frees `VsDragoon.dat` / `VsHydra.dat` when a run ends, so a second run in the same scene loads a joint out of the freed archive and crashes in `JObj_LoadJoint`. A second latch, cleared with the others at `On3DLoadStart`, holds one bit per legendary. A registered machine's archives are reloaded per run and carry no such limit.
...
- `SetDeathHandler` is how a consumer sees KOs. This mod replaces the engine's only `bl Ply_AddDeath`, in `Machine_GiveDamage`, to count them per widened kind, so a second mod cannot replace the same call - it hands the KO on instead, after the engine's own bookkeeping has run, with the victim, the victim's `DmgLog` and the widened kind that was being ridden.
...
- `GetSelectIconMax()` is the real cap on appended characters, and `GetGridCols()` is what any code walking the character grid itself has to use instead of a literal 10.
...
- **The widened list overruns live per-player state.** Packing clears `base+0x66` through `base+0x86` on both screens, which covers the per-slot save-file index and pending latch arrays their think functions read, and the relocated flags sit on the first byte of one array each. This is unconditional - it happens with no custom machine registered - and the only fix that removes it without moving each screen's whole block out of `GameData` is capping the list at the vanilla 20 entries.
