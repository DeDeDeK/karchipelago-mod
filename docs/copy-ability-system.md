...
2. **`revertKirbyModel_`** (0x801a7d70, map name `revertKirbyModel?`) — frees the ability JObj (+0x8fc) and its heap alloc (+0x904) and nulls both, calls the teardown core, then zeroes the ability callback fields +0x92c, +0x930, +0x934, +0x938, +0x7fc, +0x7e8 and +0x7ec.
3. **`Rider_TeardownCopyAbility`** (0x801a810c) — the core: sets `copy_kind` = -1, spawns the "ability lost" poof effect (`Effect_SpawnSync` 0x80236c40, effect id `0x3a990`), plays the loss SFX, and applies the fade ColAnim (`0x2c`). Poof effect details are in `effects-system.md`.
...
To discard the held ability from a mod (a manual "drop ability" control, a trap), reproduce the expiry structure but use the universal remover so it works for any kind:
