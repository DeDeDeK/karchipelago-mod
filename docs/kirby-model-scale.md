...
Kirby model up (x1.5) or down (x0.5), easing in over ~1 second rather than
...
multiplies into the model's transform **every frame**. Writing that field
(`mods/archipelago/src/kirby_scale.c`) is the entire mechanism - no JObj poking
required - and the change naturally persists until the object is recreated
(scene change), at which point it resets to 1.0.
...
- **Multiplicative, clamped.** Each Big x1.5, each Small x0.5, applied to the
