...
- One subtext is bounded by **two** limits inside `Text_ConvertASCIIToShiftJIS` (0x8044fb0c), which both `Text_SetText` and `Text_AddSubtext` route through. The first is its input: it stops reading after 128 bytes, so sanitized text is capped at `TEXTBOX_RUN_BYTES` (127). The second is its output, and it is the tighter one. The converter writes into the 128 bytes immediately below the input buffer it is still reading (output at `r1+120`, input at `r1+248` in `Text_SetText`'s frame) while emitting *more* bytes than it consumes - a letter costs three, a `TEXTCMD_POSPUSHEND` plus its 2-byte code, and a digit entering tight-spacing mode pays a 5-byte `TEXTCMD_POSPUSH` first. Once a run's output runs more than `TEXTBOX_CONVERT_SLACK` (128) bytes ahead of its input, the write pointer overtakes the read pointer and the converter re-reads its own output as text, emitting garbage for the rest of the run. `TextBox_ConvertCost` predicts that output, and `TextBox_SetRun` scales its request by whichever limit is over until both fit, returning how many characters actually landed. Every caller works from that return value - measuring text the engine silently dropped or corrupted would report a fit for a run that never rendered.
