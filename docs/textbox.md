...
The canvas is the hoshi ortho screen camera (640×480 raw pixels, created by `ScreenCam_Create` inside hoshi's `Hook_SceneChange`). Messages are allocated with `Hoshi_CreateScreenText`. `TEXTBOX_MARGIN` (10px) keeps the stack off the edges.
...
| Spacing | Tight / Normal / Wide | Tight | Extra gap 0 / 0.25 / 0.5 × rendered text height; reflows live |
...
Top Ride's post-render callback runs a second `HSD_StartRender` pass (`TopRide_CustomRenderer`, `0x80286d7c`) that overwrites the EFB and wipes screen-canvas overlays every frame. `TextBox_TopRideReRender`, hooked at `0x80009084` (the instruction right after the `bl TopRide_CustomRenderer` inside `TopRide_PostRenderCallback`), walks the `stc_textcanvas_first` list and re-issues `CObjThink_Common` on each canvas's `cam_gobj` to redraw on top. Any mod with a Top Ride HUD or text overlay needs the same treatment — see `topride-system.md`.
