...
One `GameClearData` (0xF4 bytes, defined in `externals/hoshi/include/game.h`) per game
mode, embedded in `GameData` at `+0xd68` (Air Ride), `+0xe80` (Top Ride) and `+0xfa8`
(City Trial), each immediately followed by that mode's cross-game records tail at `+0xF4`.
It carries the two checkbox-filler counters, `grid_mapping[120]` (clear_kind -> visual grid
position, `col = pos % 12`, `row = pos / 12`), and `clear[120]`, one status byte per
checkbox. `gmGetClearcheckerTypeP` (`0x800076a0`) is the accessor and asserts for
`mode >= 3`.

Almost everything below turns on the five meaningful bits of a `clear[]` byte:
`is_new` (0x01, objective completed, pending the acknowledgement animation), `is_filler`
(0x02, a checkbox filler was spent on this slot), `is_unlocked` (0x04, raised after the
unlock animation has played), `has_reward` (0x08, set by
`Checklist_SetRewardFlagOnUnlocks` on an unlocked slot that carries a reward) and
`is_visible` (0x10, the cell is drawn on the grid at all). The top three bits are unused.
...
### Checklist_UpdateCellInfo hover flow (`0x80181d70`)

The per-frame hover tooltip for the selected checkbox, and the function the mod's
cross-mode display hooks all live inside. Each frame it reverse-scans `grid_mapping` for
the cursor's visual position to get a clear_kind, and short-circuits if that matches the
clear_kind it cached last frame. Otherwise it scans the current mode's reward table for a
row whose `clear_kind` matches, verifying with `ClearChecker_CheckUnlocked`, and leaves
`r27` = reward_index or -1 (`0x80181ee4`-`0x80181f5c` - the block the mod replaces
wholesale).

It then compares that result against the previous frame's. No-reward -> reward starts the
icon animation at `0x80181fc8` and resets a display-state counter to 0; reward -> a
...
checkboxes appear - the same order `archipelago_api.h`'s `AP_REWARD_*` enum uses). The
game's internal reward table (`stc_reward_table_ptrs`) is in a different, ROM-defined
order, and the entire mod
...
`ChecklistRewards_OnBoot` runs `AllocateRewardTables` first (the index maps must exist
...
`ChecklistRewards_OnSaveLoaded` calls `RebuildRewardTablesFromShuffle` (rebuild each
`stc_reward_table_ptrs[m][i].clear_kind` and `cross_mode_slots` from the saved u16 arrays)
...
as 2x `u64` to cover the 0..119 clear_kind range) and forwards new bits as AP location
