...
Every machine can be individually locked behind an Archipelago unlock item. AP item `AP_MACHINE_UNLOCK_BASE` (830) + an unlock mask bit routes through `ap_item_handler.c` to `GateMachines_UnlockMachine(bit, announce)`, which sets that bit in `APSave.machine_unlocked_mask` and posts a textbox. One mask covers all three modes: City Trial field spawns and select grids, the Air Ride character select, and the Top Ride lobby's Free Star / Steer Star control types, so unlocking a machine once makes it available everywhere and the AP pool holds one item per machine rather than one per mode.

**File:** `mods/archipelago/src/gate_machines.c`.

## The Kind Space

The 26 `MachineKind`/`VCKIND` values (`VCKIND_NUM` = 26, `machine.h`), one bit each, then bit 26 (`AP_MACHINE_BIT_AP_STAR`) for the Archipelago Star at whatever `MachineKind` the registry handed it. Any other machine `custom_machines` registers has no bit and no unlock item, and is always available: the apworld ships unlock items only for the machines it knows. Six VCKINDs have no `CharacterKind` and are city-spawn or transformation only: FREE, STEER, WINGKIRBY, WHEELNORMAL, WHEELKIRBY, WHEELVSDEDEDE. Character-to-machine resolution goes through `CharacterDesc_GetMachineKind` (`menu.h`), which un-does the bike-relative encoding of `CharacterDesc.machine_kind` (bikes store a 0-6 slot against VCKINDs 19-25).

`custom_machines` is a required dependency. It owns the widened kind space and the engine seams widening breaks - both character select screens' icon packing, the City Trial field spawn roll, and the legendary assembly cutscene - and this mod reimplements none of them: it hands the registry a filter for each and lets it call back. Registered machines, stars or bikes, take `MachineKind`s past 26 in discovery order.

Every ceiling in `gate_machines.c` is the runtime `MachineKind_Num()` / `CharacterKind_Num()` rather than `VCKIND_NUM` / `CKIND_NUM`, and every `(is_bike, class slot)` to `MachineKind` conversion goes through `MachineKind_Resolve()`, because a registered machine's `MachineKind` is not its class slot plus a fixed base. Those are thin bindings in `main.h` over inlines the registry publishes in `custom_machines_api.h`, which is also where their vanilla fallbacks live.
...
City Trial's Free Run mode does not use the weighted spawner. `CityMachineSpawn_Think` (0x801df840) branches on `Gm_GetCityMode()` and, in Free Run, calls `CityMachineSpawn_SpawnFreeRunMachine` (0x801dee58) -> `CityMachineSpawn_PickFreeRunKind` (0x801de41c), which fills the city with up to 23 machines by drawing uniformly from the kinds not yet placed. It tracks those in `MachineSpawnData.freerun_placed[VCKIND_NUM]` (+0xac), which `CityMachineSpawn_Init` (0x801ddee8) zeroes and then seeds by walking the five player slots and incrementing the entry for each `PKIND_HMN` / `PKIND_CPU` slot's `Ply_GetVehicleKind` - so nobody's starting machine is ever duplicated on the field, and a session with one unlocked kind leaves the picker no candidate at all. It takes every remaining kind unconditionally - except 4 (Hydra) and 8 (Dragoon), which it puts through `CityTrial_CheckLegendaryMachineUnlocked` (0x8000c508), a **checklist** reward query that AP never writes.

Two patches make the mask that roster's authority. A `CODEPATCH_REPLACECALL` at 0x801de528 answers the checklist query from `GateMachines_CheckFreeRunKindUnlocked`, and a `CODEPATCH_REPLACEINSTRUCTION` at 0x801de518 widens the `beq` guarding it (`cmpwi r25, 4` / `cmpwi r25, 8`) into an unconditional `b 0x801de524`, so all 26 kinds reach the call rather than only the two legendaries. The picker's own candidate/exclude bookkeeping is untouched: an answer of 0 marks the kind excluded in the stack array, 1 counts it into `r24` for the `HSD_Randi` draw. The city therefore holds one machine per unlocked kind instead of the whole roster.

The vanilla per-kind spawn-table check ahead of it (`MachineSpawnData +0x2c + kind*4 == -1`) still runs first, so a kind Free Run never placed anyway stays unplaced whatever the mask says. With no kind passing, `r24` stays 0, the picker returns its two outputs at -1, and `CityMachineSpawn_SpawnFreeRunMachine` skips creation - an empty city, not a crash. The starting machine is unaffected: it comes from the gated select grid, not this placer. The picker's 0-25 loop cannot reach a custom `MachineKind` at all, so the Archipelago Star never appears in Free Run however its bit reads.
...
`RandomUnlockedKirbyCKind()` picks a random unlocked CharacterKind for the free-roam Trial start but **excludes `CKIND_DEDEDE` and `CKIND_METAKNIGHT`**: their riders rely on rider-specific 3D HUD assets that vanilla's HUD loader short-circuits in Base CT (`major==CITY && cityMode==TRIAL`), so selecting them there would NULL-deref `3DHud_CreateSpeedometerInner` during scene init. It falls back to `CKIND_COMPACT` when no eligible Kirby-rider machine is unlocked. Stadium and Free Run are unaffected by the exclusion, since nothing re-picks there and Dedede/Meta Knight are valid in those contexts.
...
- **RaceInit** deliberately lands past the `panel_pkind` CPU-fill loop at 0x8002d710..0x8002d744 rather than right after the `panel_machine` reset; landing inside that loop would clobber its caller-saved iterator `r7`. Hooking at the post-loop `bl Gm_GetGameData` is clean - the framework's auto-re-execution of the `bl` reloads `r3 = GameData*` for the following `addi r6, r3, 407`, so no epilogue is needed. Nothing between the reset block and the hook site reads `panel_machine[]`.
...
The icon grid is gated by the same availability filter as City Trial's, through the packing `custom_machines` owns, plus one `CODEPATCH_REPLACEFUNC` at `GateMachines_OnBoot` for the title demo:

1. **Availability.** Vanilla's per-character answer is `AirRide_CheckCharacterAvailable` (0x8002090c), which maps each ckind to a checklist reward index, makes only Warp Star available by default, and hardcodes four ckinds unavailable whatever the save holds - its jump table at 0x80496e10 sends 0 (Compact Star), 15 (Dragoon), 16 (Hydra) and 17 (Flight Warp Star) to a bare `li r3, 0` with no reward query. `custom_machines` still asks it for vanilla kinds and hands the answer to `GateMachines_FilterSelectCharacter` as `default_available`, which discards it for `IsCKindUnlocked`. Dropping those four exclusions is what makes an owned City Trial machine rideable in Air Ride. The function is not replaced: its only engine callers are inside `AirRide_PopulateSelectIcons`, which `custom_machines` replaces, so a build without the registry keeps vanilla's answer and vanilla's packing together. Nothing else has to move: the icon grid at 0x80495800 places Dragoon and Hydra at row 0 columns 0 and 9, and the `MnSelplyAll` icon TexAnim carries 20 distinct 64x64 CMPR images, one per ckind. Vanilla's reorder block that treated ckinds 15/16/18/19 as row-end specials does not run at all, because the replacement below packs left-aligned with no reorder.
...
**Real in-game CPU machine pick.** The actual Air Ride CPU machine is chosen in `loadCPU` (0x80023600) and its sibling setup paths, which index a random entry out of the available-character list the packer builds through `GateMachines_FilterSelectCharacter`. CPUs therefore already draw a random unlocked machine with no separate machine-pick hook. CPU color is gated on its own path in `gate_colors.c`.
...
- Each vanilla legendary at most once per scene. The engine frees `VsDragoon.dat` / `VsHydra.dat` when a run ends, so a second run in the same scene loads a joint out of the freed archive and crashes in `JObj_LoadJoint`. A registered machine's archives are reloaded per run and carry no such limit.
...
`u32 machine_unlocked_mask` in `APSave` (`main.h`, accessed via the global `ap_save`) - bit N = vanilla `MachineKind` N, and bit 26 (`AP_MACHINE_BIT_AP_STAR`, `archipelago_api.h`) the Archipelago Star. It is exposed through `ArchipelagoAPI` as `AP_UNLOCK_MACHINE`, and when the slot option `machine_gating_enabled` is 0, `APOptions_ApplyUngatedCategories` (`main.c`) pre-fills it at connect with all `AP_MACHINE_BIT_NUM` (27) bits.

AP item ID = `AP_MACHINE_UNLOCK_BASE` (830) + mask bit, so 830-854 are the vanilla machines and 856 (`AP_MACHINE_UNLOCK_AP_STAR`) the star. The handler accepts 830..856 minus 855, then calls `GateMachines_UnlockMachine(bit, /*announce=*/1)`. All in-range IDs are accepted defensively even though the apworld generates fewer. `checklist_rewards.c` calls the same entry point with `announce = 0` for the machine rewards, passing the vanilla kind as its bit.

**The star is bound by name, not by position.** `custom_machines` hands out appended kinds in FST discovery order, so the star's `MachineKind` depends on which other drop-in machines a build carries. `IsKindUnlocked` asks for a kind's bit through `GateBit`: a vanilla kind is its own bit, the kind `GateApStar_MachineKind()` answers is bit 26, and any other registered kind has no bit and reads unlocked. `GateApStar_MachineKind` binds at `OnSaveLoaded` and caches the kind, since the spawn and select filters ask per kind. The star's bit is set and saved even in a build without `ap_star`.

`GateMachines_UnlockMachine` sets the bit, logs, and (when announcing) enqueues `"Unlocked Machine: <name>"` with `tb_api->MachineColor`, naming bit 26 `AP_STAR_MACHINE_NAME`. `VCKIND_WHEELDEDEDE` and `VCKIND_WINGMETAKNIGHT` instead announce `"Unlocked Character: King Dedede"` / `"Unlocked Character: Meta Knight"`, matching the checklist reward path.

**IDs the apworld does not generate** (`worlds/kirby_air_ride/KARItems.py`): 847 `WINGKIRBY`, 849 `WHEELNORMAL`, 850 `WHEELKIRBY` - transformation/ability-state forms with no readers anywhere, so granting them would be a genuine no-op - and 855 `WHEELVSDEDEDE`, which the handler rejects. The remaining 22 vanilla IDs and 856 are all generated as `progression`.

**A seed without Top Ride gets Free and Steer for free.** One mask covers all three modes, so `machine_gating_enabled` stays 1 for a seed that drops Top Ride but keeps City Trial or Air Ride, while the AP world ships the two Top Ride unlocks only when Top Ride is in the seed - the lobby would stay locked for the whole run. `APOptions_ApplyUngatedCategories` (`main.c`) therefore ORs `TR_MACHINE_BITS` into the mask at connect when `goal[GMMODE_TOPRIDE]` is `GOAL_NONE`, matching what the Top Ride courses and items already do through their own ungated pre-fills. The reverse case needs nothing: with City Trial and Air Ride both out, the AP world reports machine gating off and every bit is pre-filled.
