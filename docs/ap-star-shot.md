...
The whole feature is `mods/ap_star/src/ap_star_shot.c`, behind the **Sphere Shot** toggle in the
...
host: its vtable `init` (`PlasmaSpread_Init`, `0x8022691c`) is a bare `blr`, three of its four state
callbacks (`PlasmaSpread_State0_Fn0` `0x802269f4`, `_Fn1` `0x802269f8`, `_Fn3` `0x80226abc`) are
`blr`, and it carries no per-kind scratch that a
...
Owner exclusion stays on - `desc.owner_gobj` is `rd->gobj`, the rider GObj, which is what
...
despawn vtable slot (`PlasmaSpread_Despawn`, `0x80226b1c`) is a bare `GObj_Destroy`: a shot that
simply ran out of life would
...
`Projectile_Create` reaches the model through the `ProjModelBlock` at the kind's `ProjKindData+0x08`
(read at `0x8021f5c0`-`0x8021f604`): `tree` is the root `JOBJDesc`, and the top byte of `flags` is how
many joints that tree holds, landing at `proj+0x1c`. `Projectile_CollectWeaponParts` (`0x80221914`)
counts the loaded tree in preorder and asserts twice - `weapon parts num over` above 10 joints, and
`weapon parts num not match` on any disagreement with that byte.

So the swap is: point `model_desc` at a static `ProjModelBlock` holding the shot's root and a count
byte of 2, keeping the original `flags`' low three bytes; call `Projectile_Create`; put the original
pointer back. The kind data itself is reached through `proj_kind_data`, the `ProjKindData **`
at `0x8055a9a8` declared in `externals/hoshi/include/projectile.h`. The window is one
synchronous call, so no other projectile can see it. `kind_data+0x10`
...
`fn2` (`PlasmaSpread_State0_EnvCollide`, `0x802269fc`) stays in place and bursts it on a steep
environment contact.
...
- `state_fn2` replaces the kind's own, which would burst the shot on the first rise a projectile
  ...
  Only the burst decision is replaced: the kind's `fn2` opens with `Projectile_UpdateEnvColl`
  (`0x80221fd4`), which drives `mpColl_Update` and `mpColl_UpdateShapeExtents` over the shot's
  `coll_data`, so the replacement calls it first. Without that a ground shot carries a stale
  collision volume and stops breaking the destructible city geometry an air shot breaks.
