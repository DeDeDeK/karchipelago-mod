...
The `CharacterDesc` table, the grid table at `0x80495800`, the linear strip at `0x804957ec` and the star class's slot-to-`CharacterKind` map at `0x80495850` sit back to back with no slack, so a 21st CharacterKind cannot be appended in place. The `custom_machines` mod relocates the first three into wider mod-owned copies by rewriting the `lis`/`addi` pair inside each accessor, and replaces the map's reader `Machine_GetCKind` (`0x8000b9f4`) outright, since its bike half at `r13 - 0x7fbc` is reached by no `lis`/`addi` pair. It also writes the grid's runtime column count into `SelIcon_GetCKind`'s `mulli r5, r0, cols` at `0x8000b9c4` (10 in vanilla, one more column per two appended characters).
...
The ipos userdata, typed in hoshi's `scene.h` as `AirRideSelectIposData` / `CitySelectIposData`, is laid out the same way on both screens up to the positions and differently after them: 20 anchor `JObj` pointers at `+0x10`, one `Vec3` per icon at `+0x60`, then Air Ride's shared icon scale at `+0x150` and count at `+0x15c` against City Trial's count at `+0x150` and scale at `+0x154`.
...
`gd[0x16f]` / `gd[0x170..]` are the count and body of the available-character list. Vanilla's `AirRide_PopulateSelectIcons` builds it through `AirRide_CheckCharacterAvailable`; `custom_machines`' replacement builds it through the consumer's availability filter. Either way the CPU picks a random **available** machine here - gating the available list is sufficient, no separate CPU hook is needed for the machine.
