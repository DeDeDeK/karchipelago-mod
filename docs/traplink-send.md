...
- **Receive**: When the AP client sets `ap_data->traplink_receive = 1`, `TrapLink_PerFrame` dispatches a mode-appropriate trap effect on every human player. The GOBJ is installed by `TrapLink_On3DLoadEnd` (Air Ride / City Trial) and `TrapLink_OnTopRideLoad` (Top Ride).
...
- The `ap_menu_settings.traplink_enabled` menu toggle is checked in `TrapLink_Send(kind)`, so individual hooks don't need to check it. The toggle also gates GObj installation: `main.c`'s `On3DLoadEnd`/`OnTopRideLoad` only call `TrapLink_On3DLoadEnd`/`TrapLink_OnTopRideLoad` when it is set, so the receive proc isn't even created while TrapLink is off.
