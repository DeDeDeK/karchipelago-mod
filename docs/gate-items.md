...
The six Archipelago Star spheres follow the same shape one category over. They are custom City Trial items, not `ItemKind`s, so they get their own `u8 ap_star_piece_unlocked_mask` rather than bits of this one - `ITUNLOCK_NUM` has all but filled a `u32` - and their own `AP_UNLOCK_AP_STAR_PIECE` category and AP item IDs 820-825. They ride the same `item_gating_enabled` flag, and `GOALGATE_AP_STAR_PIECES` holds them out of the ungated pre-fill the way `GOALGATE_LEGENDARY_PIECES` does the vanilla pieces. The mask is owned by `ap_star_pieces.c`, which is where the spheres are enabled or held out of the item registry.
