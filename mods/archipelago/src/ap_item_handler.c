...
int APItems_HandleItem(uint ap_item_id)
{
    ...
    // Six VCKINDs are not generated by the AP world (FREE, STEER, WINGKIRBY,
    // WHEELNORMAL, WHEELKIRBY, WHEELVSDEDEDE) — see docs/client-game-protocol.md.
    // The mod still accepts them defensively if it ever sees them; for those
    // bits no game code consults the bitfield, so the effect is a no-op.
    ...
    // Dedede stadium's CPU-only machine; nothing reads its bit.
