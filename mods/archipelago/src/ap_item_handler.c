...
int APItems_HandleItem(uint ap_item_id)
{
    ...
    // Five VCKINDs are not generated by the AP world (FREE, STEER, WINGKIRBY,
    // WHEELNORMAL, WHEELKIRBY) — see docs/client-game-protocol.md. The mod
    // still accepts them defensively if it ever sees them; for those bits no
    // game code consults the bitfield, so the effect is a no-op.
    // VCKIND_WHEELDEDEDE (24) is the player-facing Dedede machine and the
    // canonical Dedede unlock. VCKIND_WHEELVSDEDEDE (25) is the Vs. King
    // Dedede stadium's CPU-only machine and is *not* part of this range —
    // the upper bound stops at it so an erroneous ID 855 falls through to
    // the unknown-item path instead of flipping a dead bit.
