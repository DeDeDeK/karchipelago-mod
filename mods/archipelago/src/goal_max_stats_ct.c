...
// Per-frame proc: if every PATCHKIND_NUM stat has the slot's patch-cap target
// worth of patches, latch the sticky save flag and re-run goal evaluation.
// Idempotent - once the flag is set, the proc early-outs.
static void GoalMaxStatsCT_PerFrame(GOBJ *rg)
{
    ...
    {
        // Stats spawn at PatchCap_GetStatStart(i) (-2, or 0 for HP) and the
        // target counts patches collected, so "target patches on this stat" is
        // start + target. Without the offset every non-HP stat would need two
        // extra patches versus HP.
